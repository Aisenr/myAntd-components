...
## 对代码的补充修改：
BmapCoordSys.js中，调用bmap.centerAndZoom前，检查地图的center和zoom是否有变化，避免调用bmap.centerAndZoom导致moveend事件频发触发。
```
    if (center && zoom && bmapModel.centerOrZoomChanged([bmap.getCenter().lng, bmap.getCenter().lat], bmap.getZoom())) {
        var pt = new BMap.Point(center[0], center[1]);
        bmap.centerAndZoom(pt, zoom);
    }
```
添加了mapOptions配置项，作为BMap.Map构造函数的第二个参数
```
    bmap: {
      roam: true,
      mapOptions: {
...
```
```
      var bmap = bmapModel.__bmap = new BMap.Map(bmapRoot, bmapModel.option.mapOptions || {});
